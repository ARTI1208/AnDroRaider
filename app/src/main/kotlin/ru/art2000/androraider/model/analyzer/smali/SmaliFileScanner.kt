...
class SmaliFileScanner(val project: ProjectAnalyzeResult, var smaliClass: SmaliClass, var onlyClass: Boolean = true) :
    ...
    override fun visitMethodParameterType(ctx: SmaliParser.MethodParameterTypeContext): SmaliClass {
        ...
        if (ancestor is MethodDeclarationContextWrapper) {
            ...
            ancestor.smaliMethod.parametersInternal.addAll(parseCompound(ctx.text)
                    .mapNotNull {
...
//        println("Error $node, line=${node.symbol.line} range=${node.symbol.startIndex}..${node.symbol.stopIndex} text=${node.text} for $smaliClass")
    ...
    override fun visitClassName(ctx: SmaliParser.ClassNameContext): SmaliClass {
        val currentClass = project.getOrCreateClass(ctx.text)
        if (currentClass == null) {
            smaliClass.ranges.add(Error(ctx.textRange, "Class name ${ctx.text} is invalid"))
        } else {
