...
interface Indexer<FAR : FileAnalyzeResult, S : IndexerSettings> {
     ...
     */
    fun analyzeFile(project: AndroidAppProject, file: File, settings: S): FAR

    /*
    Return RxJava Observable so that user can provide it's own event listeners
    */
    fun analyzeFilesInDir(project: AndroidAppProject, directory: File, settings: S): Observable<FAR>

    fun indexProject(project: AndroidAppProject, settings: S): Observable<FAR>
}