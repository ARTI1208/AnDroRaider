...
//        project.rootPackage.subpackages.addAll(project.packages)
        ...
        project.packages.mapNotNull { it.exists() }.sortedBy { it.fullname }.forEach {
    ...
    private fun addChildren(item: TreeItem<SmaliComponent>) {
        ...
        if (component is SmaliPackage) {
            component.subpackages.mapNotNull { it.exists() }.sortedBy { it.fullname }.forEach {
                val newItem = TreeItem<SmaliComponent>(it)
                item.children.add(newItem)
                addChildren(newItem)
            }
            component.classes.mapNotNull { it.exists() }.sortedBy { it.fullname }.forEach {
                val newItem = TreeItem<SmaliComponent>(it)
                item.children.add(newItem)
            }
        }
